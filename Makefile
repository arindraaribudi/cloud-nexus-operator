...
docker-buildx-setup: ## Create a multi-platform buildx builder if not already present.
	$(CONTAINER_TOOL) buildx inspect frp-builder >/dev/null 2>&1 || \
...
docker-build-operator: docker-buildx-setup ## Build and push the crd-tunnel-operator image.
	$(CONTAINER_TOOL) buildx build \
		--builder frp-builder \
		--platform $(PLATFORMS) \
		--push \
...
# Legacy single-platform build for local testing.
.PHONY: docker-build-local
docker-build-local: ## Build manager image locally (single-platform, no push).
	$(CONTAINER_TOOL) build -t $(REGISTRY)/crd-tunnel-operator:$(OPERATOR_VERSION) .
